from utils import load_data, load_template, add_anotacao, build_response, load_nota
...
def edit(request, id):
    if request.startswith('POST'):
        ...
        for chave_valor in corpo.split("&"):
            ...
            if spaces[0] == "detalhes":
                params["detalhes"] = unquote_plus(spaces[1])
        nota = Note(id=id, title=params["titulo"], content=params["detalhes"])
        db.update(nota)
        return build_response(code=303, reason='See Other', headers='Location: /')
    
    nota = load_nota(id)
    return build_response(body= load_template("update.html").format(title=nota.title, content = nota.content, id = nota.id))
